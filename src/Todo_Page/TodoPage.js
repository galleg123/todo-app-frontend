                    ...
                    {todos.map((todo) => {
                        return (
                            <div className={styles.todo} key={todo.id}>
                                <h1 className={styles.todoTitle} style={{ textDecorationLine: todo.status && 'line-through', textDecorationStyle: 'solid' }} onClick={(event) => handleTaskDone(event, todo.id, todo.status)}>{todo.title}</h1>
